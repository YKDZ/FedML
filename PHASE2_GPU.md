...
已完成: *Draft PHASE2_GPU.md planning document* (1/1)

以上是完整的 PHASE2_GPU.md 规划文件。文档覆盖了：

- **GPU 运行档位定义**（`single-gpu-deterministic` / `single-gpu-fast` / `multi-gpu-throughput`）
- **代码改动清单**（6 个现有文件修改 + 10+ 个新建文件）
- **9 步实施流程**（从环境预检到结果汇总）
- **完整验收标准**（8 大类共 30+ 条检查项）
- **学术门槛对齐**（M1/M2/M3 的 GPU 正式实验矩阵和量化门槛）
- **GPU 特有风险与注意事项**（确定性兼容性、VRAM 估算、MPI 竞争、cpu_transfer 必要性） 
