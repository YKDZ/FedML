...
2026-03-26 15:45 启动，当前正在运行 experiment 1/24（α=0.1 seed=0）。

> **预期**：epochs=5 应使每轮本地训练的梯度步数增加 5 倍，预计 MA 提升 5-10%，足以覆盖门槛差距。单个实验耗时预计从 ~17 分钟增至 ~60-80 分钟（epochs 5× 但 GPU 计算不是瓶颈，MPI 通信开销恒定）。
...
| M1 local epochs | `epochs: 1` | `epochs: 5` | epochs=1 下全部 α 不达标，差距最大 17.8% |
...
## 8. 当前实验状态（2026-03-26 20:30 快照）

| 会话 | GPU | 脚本 | 进度 | 状态 |
|---|---|---|---|---|
| m1 | GPU 0 | run_m1_baseline_gpu.sh (epochs=5) | 4/24 | 🔄 运行中（α=0.3 seed=0） |
| m2 | GPU 1 | run_m2_priority_gpu.sh | 36/36 | ✅ 已完成（18:58:40） |
| m3 | GPU 3 | run_m3_priority_gpu.sh | 36/36 | ✅ 已完成（18:41:43） |

**结果文件总数**：76（含 M1 epochs=5 的 4 个新文件）

**M1 已完成的 epochs=5 实验**：
- ResNet18/CIFAR10: α=0.1 seed={0,1,2}，α=0.3 seed={0}（共 4/24）
- 单个实验约 65 分钟，剩余 20 个约需 ~21 小时

**日志文件**：
- M1: `/data/home/ykdz/m1_gpu_e5.log`
- M2: `/data/home/ykdz/m2_gpu2.log`（已结束）
- M3: `/data/home/ykdz/m3_gpu.log`（已结束）
...
- [ ] 等待 M1 epochs=5 完成（预计 ~21 小时），验证是否达到学术门槛
- [ ] 考虑将空闲的 GPU 1 和 GPU 3 分担 M1 剩余实验，将时间从 ~21h 缩短至 ~7h

### M2 补救（无目标攻击未达标）

- [ ] 决定补救策略：增大 PMR（30%/40%）/ 增加 rounds 到 100 / 阈值校准
- [ ] 根据策略补充实验，特别是：
  - byzantine CIFAR10 α=0.5（当前 67.99%，需降至 ≤40%）
  - byzantine/label_flipping MNIST（当前 95~98%，远超 50% 门槛）
  - label_flipping CIFAR10 α={0.1,0.5}（46%/78%，需降至 ≤40%）
- [ ] 重新审视 model_replacement 的 FedML 实现，确定是否为实现 bug 或需调参

### M3 跟进

- [ ] 补做无攻击场景下防御保真度实验（验证 MA 下降 ≤ 2% 门槛）
- [ ] 考虑补充 rfa 和 cclip 防御的实验（当前仅跑了 krum 和 trimmed_mean）
- [ ] 考虑增加 label_flipping / model_replacement 攻击下的防御实验

### 后续阶段

- [ ] 运行 `summarize_results.py` 产出汇总统计表
- [ ] 评估是否需要展开 M2/M3 全矩阵
- [ ] 补做 CPU vs GPU 对齐验证（如时间允许）
- [ ] 补做 Aggregation Time 基准测定（如时间允许）